...
def generate_bnn_layer_verilog(module_name: str, num_inputs: int, num_nodes: int, input_bitwidth :int, layer_no:int) -> str:
...
"""
    ...
    for node in range(num_nodes):
        for i in range(num_inputs):
            verilog += f"    wire [{m-1}:0] weighted_inputs{node+1}_{i}_0, weighted_inputs{node+1}_{i}_1;\n"
...
"""
    ...
    for node in range(num_nodes):
        for i in range(num_inputs):
            ...
            verilog += (
                f"    weighted_inputs_1 w{idx} (.inputs(inputs{i}_{layer_no}), .w(w{node+1}_0_{layer_no}[{i}]), .wi(weighted_inputs{node+1}_{i}_0));\n"
                f"    weighted_inputs_1 w{idx}_bar (.inputs(inputs{i}_{layer_no}), .w(w{node+1}_1_{layer_no}[{i}]), .wi(weighted_inputs{node+1}_{i}_1));\n"
            )
    ...
    for node in range(num_nodes):
        verilog += (
            ...
            f"    add{adder_bit}bit_{layer_no} u{node+num_nodes} (.a(sum2[{node}]), .b(b{node+1}_{layer_no}), .cin(1'b0), .y(biased_sum2[{node}]));\n"
            f"    add{adder_bit}bitbar_{layer_no} ub{node} (.a(sum1bar[{node}]), .b(b{node+1}_{layer_no}), .cin(1'b0), .y(biased_sum1bar[{node}]));\n"
            f"    add{adder_bit}bitbar_{layer_no} ub{node+num_nodes} (.a(sum2bar[{node}]), .b(b{node+1}_{layer_no}), .cin(1'b0), .y(biased_sum2bar[{node}]));\n"
        )
   ...
   # Display block
    ...
    for phase in (0,1):
        ...
        sig_w = ", ".join(f"w{j+1}_{phase}_{layer_no}" for j in range(num_nodes))
        verilog += f'        $display("Weights{phase}: {fmt_w}", {sig_w});\n'
...
def generate_boolean_to_arithmetic_verilog(input_bitwidth: int, layer_no:int ) -> str:
    verilog = []
    verilog.append("`timescale 1ns/1ps\n")
    verilog.append(f"module mux_{layer_no} (\n")
    verilog.append("    input  wire a, b, s,\n")
    verilog.append("    output wire y\n);\n")
    verilog.append("assign y = (~s & a)|(s & b);\nendmodule\n\n")

    verilog.append(f"module boolean_arithmetic_coversion_{layer_no} (\n")
    ...
    for i in range(1, input_bitwidth):
        if i == 1:
            ...
            a = f"y0[{i-1}] ^ y0[{i}]"
            b = f"y0[{i}] ^ y1[{i-1}]"
        verilog.append(f"    mux_{layer_no} m{i}(.a({a}), .b({b}), .s(arith_share0[{i-1}]), .y(arith_share0[{i}]));\n")
...
def generate_activation_and_conversion_module(num_inputs: int, input_bitwidth: int, num_nodes: int,layer_no :int ) -> str:
    ...
    def weights_decl():
        return "\n  ".join([f"input  wire [{weight_bitwidth-1}:0] w{j}_0_{layer_no}, w{j}_1_{layer_no}," for j in range(1, num_nodes+1)])
    ...
    def arith_mask_inputs():
        return "\n  ".join([f"input wire [{mask_bits-1}:0] ar{j}, ar{j}bar," for j in range(num_nodes)])

    def masked_activation_outputs():
        return "\n  ".join([
            f"output wire masked_activation{j}_{layer_no}, masked_activation{j}bar_{layer_no},\n  output wire mask{j}_{layer_no}, mask{j}bar_{layer_no},"
            for j in range(num_nodes)
        ])

    def arithmetic_outputs():
        lines = []
        for j in range(num_nodes):
            line = f"output wire [{act_bits-1}:0] act{j}_0_{layer_no}, act{j}_1_{layer_no},\n  output wire [{act_bits-1}:0] act{j}_0bar_{layer_no}, act{j}_1bar_{layer_no}"
            if j != num_nodes - 1:
                line += ","
            lines.append(line)
        return "\n  ".join(lines)
    ...
    def activation_array_instance(name, suffix):
        ...
        lines += [f"    .mask{j}(mask{j}{suffix_str}_{layer_no})," for j in range(num_nodes)]
        lines[-1] = lines[-1][:-1]  # remove trailing comma
        lines.append("  );")
        return "\n".join(lines)

    def boolean_conv_instances():
        blocks = []
        for j in range(num_nodes):
            for suffix in ["", "bar"]:
                label = "" if suffix == "" else "b"
                blocks.append(f"""  boolean_arithmetic_coversion_{layer_no} conv{j}{label} (
    .x0(masked_activation{j}{suffix}_{layer_no}),
    .x1(mask{j}{suffix}_{layer_no}),
    .r_mask(ar{j}{suffix}),
    .arith_share0(act{j}_0{suffix}_{layer_no}),
    .arith_share1(act{j}_1{suffix}_{layer_no})
  );""")
        return "\n\n".join(blocks)
    ...
    def display_block():
        ...
        lines = [
            ...
            disp_row("mask", [f"mask{j}_{layer_no}" for j in range(num_nodes)]),
            disp_row("maskbar", [f"mask{j}bar_{layer_no}" for j in range(num_nodes)]),
            "  end\n",
            "  always @(*) begin",
            f'    $display("----- LAYER {layer_no}  arithmetic activations -----");',
            disp_row("masked_activation_arith", [f"act{j}_0_{layer_no}" for j in range(num_nodes)]),
            disp_row("mask_arith", [f"act{j}_1_{layer_no}" for j in range(num_nodes)]),
            disp_row("masked_activationbar_arith", [f"act{j}_0bar_{layer_no}" for j in range(num_nodes)]),
            disp_row("mask_arithbar", [f"act{j}_1bar_{layer_no}" for j in range(num_nodes)]),
            "  end"
        ]
        return "\n".join(lines)
...
def generate_full_design(num_inputs: int,
                         input_bitwidth: int,
                         num_nodes: int ,layer_no : int) -> str:
    parts = [
        #generate_full_adder_modules1(layer_no),
        #generate_full_adder_modules2(layer_no),
        ...
        generate_bnn_layer_verilog(f"layer{layer_no}",
        ...
        generate_boolean_to_arithmetic_verilog(input_bitwidth,layer_no),
...
def generate_output_layer_max_module(num_inputs: int,
    ...
    for j in range(1, num_nodes+1):
        lines.append(f"    input  wire [{num_inputs-1}:0] w{j}_0_{layer_no}, w{j}_1_{layer_no},")
    # biases for layer1:
    for j in range(1, num_nodes+1):
    ...
    def layer1_inst():
        ...
        # weights
        for j in range(1, num_nodes+1):
            stmt.append(f"        .w{j}_0_{layer_no}(w{j}_0_{layer_no}), .w{j}_1_{layer_no}(w{j}_1_{layer_no}),")
        # biases
        for j in range(1, num_nodes+1):
...
def generate_connector(
    ...
    k2 = 2 * num_nodes1 
    sum2_w = m + math.ceil(math.log2(k2))
    bias2_w = sum2_w 
    ...
    for i in range(num_inputs):
        lines.append(f"    input  wire [{m-1}:0] inputs{i}_{layer_no1},")
    lines.append("    // Layer-1 weights & biases")
    for n1 in range(1, num_nodes1+1):
        lines.append(f"    input  wire [{num_inputs-1}:0] w{n1}_0_{layer_no1}, w{n1}_1_{layer_no1},")
    ...
    for n2 in range(1, num_nodes2+1):
        lines.append(f"    input  wire [{k2-1}:0] w{n2}_0_{layer_no2}, w{n2}_1_{layer_no2},")
...
# if you also need the complementary bars:
    ...
    lines.append("")
# initialize them all to $random
    lines.append("  initial begin")
    for n in range(num_nodes1):
        lines.append(f"    ar{n}_{layer_no1}    = $random;")
        lines.append(f"    ar{n}bar_{layer_no1} = $random;")
    lines.append("    #1;")
    lines.append("  end\n")
...
#activation declaration
    ...
    for j in range(num_nodes1):
        lines.append(f"  wire [{input_bitwidth-1}:0] act{j}_0_{layer_no1}, act{j}_1_{layer_no1};")
    for j in range(num_nodes1):    
        lines.append(f" wire [{input_bitwidth-1}:0] act{j}_0bar_{layer_no1}, act{j}_1bar_{layer_no1};")
...
# weights & biases
    for n1 in range(1, num_nodes1+1):
        lines.append(f"    .w{n1}_0_{layer_no1}(w{n1}_0_{layer_no1}), .w{n1}_1_{layer_no1}(w{n1}_1_{layer_no1}),")
...
# masked‐boolean outputs & masks
    ...
    for n1 in range(num_nodes1):
        lines.append(f"    .mask{n1}_{layer_no1}(mask{n1}_{layer_no1}), "
                 f".mask{n1}bar_{layer_no1}(mask{n1}bar_{layer_no1}),")
                 
# arithmetic‐share randomness taps
    lines.append("    " + ", ".join(
        f"  .ar{n}(ar{n}_{layer_no1}), .ar{n}bar(ar{n}bar_{layer_no1})"
        for n in range(num_nodes1)
    ) + ",")
    # arithmetic outputs
    for n1 in range(num_nodes1):
        lines.append(f"    .act{n1}_0_{layer_no1}(act{n1}_0_{layer_no1}), "
                 f".act{n1}_1_{layer_no1}(act{n1}_1_{layer_no1}), "
                 f".act{n1}_0bar_{layer_no1}(act{n1}_0bar_{layer_no1}), "
                 f".act{n1}_1bar_{layer_no1}(act{n1}_1bar_{layer_no1}),")
    ...
    lines.append("  initial begin")
    for n in range(num_nodes2):
        lines.append(f"    ar{n}_{layer_no2}    = $random;")
        lines.append(f"    ar{n}bar_{layer_no2} = $random;")
    lines.append("    #1;")
    lines.append("  end\n")
...
#activation declaration
    ...
    for j in range(num_nodes2):
        lines.append(f"  wire [{input_bitwidth-1}:0] act{j}_0_{layer_no2}, act{j}_1_{layer_no2};")
    for j in range(num_nodes2):    
        lines.append(f" wire [{input_bitwidth-1}:0] act{j}_0bar_{layer_no2}, act{j}_1bar_{layer_no2};")
    ...
    for i in range(num_nodes1):
        lines.append(f"    .inputs{i}_{layer_no2}(act{i}_0_{layer_no1}),")
    for i in range(num_nodes1):
        lines.append(f"    .inputs{i+1*num_nodes1}_{layer_no2}(act{i}_1_{layer_no1}),")
    # weights & biases
    for n2 in range(1, num_nodes2+1):
        lines.append(f"    .w{n2}_0_{layer_no2}(w{n2}_0_{layer_no2}), .w{n2}_1_{layer_no2}(w{n2}_1_{layer_no2}),")
    ...
    for n1 in range(num_nodes2):
        lines.append(f"    .mask{n1}_{layer_no2}(mask{n1}_{layer_no2}), "
                 f".mask{n1}bar_{layer_no2}(mask{n1}bar_{layer_no2}),")
                 
    # arithmetic‐share randomness taps
    lines.append("    " + ", ".join(
        f"  .ar{n}(ar{n}_{layer_no2}), .ar{n}bar(ar{n}bar_{layer_no2})"
        for n in range(num_nodes2)
    ) + ",")
    # arithmetic outputs
    for n1 in range(num_nodes2):
        lines.append(f"    .act{n1}_0_{layer_no2}(act{n1}_0_{layer_no2}), "
                 f".act{n1}_1_{layer_no2}(act{n1}_1_{layer_no2}), "
                 f".act{n1}_0bar_{layer_no2}(act{n1}_0bar_{layer_no2}), "
                 f".act{n1}_1bar_{layer_no2}(act{n1}_1bar_{layer_no2}),")
    # strip the trailing comma on the last port
    lines[-1] = lines[-1].rstrip(',')
    # close instance
    lines.append(f"  );\n")
