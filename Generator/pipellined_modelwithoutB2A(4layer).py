...
"""
    ...
    for node in range(num_nodes):
        for i in range(num_inputs):
            idx = node * num_inputs + i
            verilog += (
                f"    weighted_inputs_2 w{idx} (.inputs(inputs{i}_{layer_no}), .w(w{node+1}_0_{layer_no}[{i}]), .wi(weighted_inputs{node+1}_{i}_0));\n"
                f"    weighted_inputs_2 w{idx}_bar (.inputs(inputs{i}_{layer_no}), .w(w{node+1}_1_{layer_no}[{i}]), .wi(weighted_inputs{node+1}_{i}_1));\n"
            )
...
# maskdeclaration
    ...
    for i in range(num_nodes1):
        lines.append(f"    .inputs{i+1*num_nodes1}_{layer_no2}(mask{i}_{layer_no1}),")
    # weights & biases
    for n2 in range(1, num_nodes2+1):
...
# maskdeclaration
    ...
    for j in range(num_nodes3):
        lines.append(f" reg maskr{j}_{layer_no3}, maskr{j}bar_{layer_no3};")
    ...
    for i in range(num_nodes2):
        lines.append(f"    .inputs{i+1*num_nodes2}_{layer_no3}(mask{i}_{layer_no2}),")
    # weights & biases
    ...
    lines.append("  always @(posedge clk) begin")
    for n1 in range(num_nodes3):
        lines.append(f"    maskr{n1}_{layer_no3} <= mask{n1}_{layer_no3};")
    for n1 in range(num_nodes3):
        lines.append(f"    masked_activationr{n1}_{layer_no3} <= masked_activation{n1}_{layer_no3};")
    for n1 in range(num_nodes3):
        lines.append(f"    maskr{n1}bar_{layer_no3} <= mask{n1}_{layer_no3};")
    for n1 in range(num_nodes3):
    ...
    for i in range(num_nodes3):
        lines.append(f"    .inputs{i+1*num_nodes3}_{layer_no4}(mask{i}_{layer_no3}),")
